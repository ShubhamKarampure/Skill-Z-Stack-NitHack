      ...
      >
        ...
        <div className="hidden md:flex items-center gap-4">
          {isAuthenticated && user ? (
            // STATE 1: LOGGED IN (Any Environment) -> Dashboard
            <Link
              href={getDashboardPath(user.role)}
              className="group relative px-6 py-3 bg-white text-black text-sm font-bold rounded-full hover:bg-zinc-200 transition-all flex items-center gap-2 shadow-[0_0_20px_rgba(255,255,255,0.3)]"
            >
              <LayoutDashboard className="w-4 h-4" />
              <span>Dashboard</span>
            </Link>
          ) : IS_PRODUCTION ? (
            // STATE 2: PRODUCTION (Not Logged In) -> Connect Wallet
            <button
            ...
            // STATE 3: DEVELOPMENT (Not Logged In) -> Login + Register
            <>
              <Link
                href="/login"
