...
export default function CandidateProfile() {
  ...
  const [verificationDetails, setVerificationDetails] = useState<any>(null); // <-- NEW
  const [isVerifying, setIsVerifying] = useState(false);

  // Form states
  const [ageThreshold, setAgeThreshold] = useState("18");
  const [credentialRoot, setCredentialRoot] = useState(
            ...
            if (credRes.success && credRes.data?.credentials) {
              ...
              setCredentials(
                credRes.data.credentials.map((c: any) => ({
                  ...
                  // Corrected: if c.isRevoked is true -> show "Revoked", else "Verified"
                  status: c.isRevoked ? "Revoked" : "Verified",
                }))
              );
            }
          }
  ...
  const handleVerifyZKP = async () => {
    ...
    setVerificationDetails(null); // reset previous
    ...
    try {
      ...
      // pick a token to verify; if none, bail
      const tokenId = credentials?.[0]?.id;
      if (!tokenId) {
        console.warn("No credential tokenId available to verify");
        setVerificationResult("error");
        return;
      }

      // Call backend ZKP verification endpoint with tokenId
      const res = await verifierService.verifyZKProof(tokenId, proofType, proof, publicSignals);

      // Store full response so we can display details in UI
      setVerificationDetails(res);

      // Extract verification object from possible response shapes
      const verificationObj = res?.verification || res?.data?.verification || res?.data;

      // Determine success/failure. Success only when all required flags are good.
      const isSuccess = !!(
        verificationObj &&
      ...
      // For a shorter rule mentioned: treat as failure if isValid is false OR isActive is false OR isRevoked is true
      const isQuickFail = verificationObj && (
        verificationObj.isValid === false ||
        verificationObj.isActive === false ||
        verificationObj.isRevoked === true
      );

      if (isSuccess && !isQuickFail) {
        setVerificationResult("success");
      } else {
        setVerificationResult("error");
      }

      // If backend indicates the credential is revoked, update the local credentials list so UI shows it
      if (verificationObj && typeof verificationObj.isRevoked !== 'undefined') {
        setCredentials(prev => prev.map(c => {
          if (String(c.id) === String(res.tokenId || tokenId)) {
            return { ...c, status: verificationObj.isRevoked ? 'Revoked' : (verificationObj.isValid ? 'Verified' : c.status) };
          }
          return c;
        }));
      }
  ...
  const handleVerifyAllCredentials = async () => {
    if (!zkpInput) return;
    setIsVerifying(true);
    ...
    try {
      ...
      const proof = proofData.proof;
      const publicSignals = proofData.publicSignals;
      // Get credRes from localStorage
      const credResRaw = localStorage.getItem("candidate_credRes");
      let creds: any[] = [];
      if (credResRaw) {
        try {
          const parsed = JSON.parse(credResRaw);
      ...
      for (const cred of creds) {
        const tokenId = String(cred.tokenId || cred.id);
        try {
          const res = await verifierService.verifyZKProof(tokenId, proofType, proof, publicSignals);
          results.push({ id: tokenId, status: res.success && res.data?.isValid ? "Verified" : "Failed" });
        } catch (err) {
          results.push({ id: tokenId, status: "Error" });
        }
      }
      alert(results.map((r) => `Credential ${r.id}: ${r.status}`).join("\n"));
    } finally {
      setIsVerifying(false);
    }
  };
                ...
                {selectedTemplate && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white/0.02 border border-white/5 rounded-2xl p-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <button onClick={() => handleCopyTemplate(selectedTemplate)} className="text-xs flex items-center gap-1 text-indigo-400 hover:text-indigo-300 transition-colors">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="p-4 bg-indigo-500/10 rounded-xl border border-indigo-500/20 mb-4">
                        {selectedTemplate === 'age' && (
                          <div>
                            ...
                            <input type="number" value={ageThreshold} onChange={(e) => setAgeThreshold(e.target.value)} className="w-full bg-black/40 border border-indigo-500/30 rounded-lg p-3 text-white focus:outline-none focus:border-indigo-500 transition-all" />
                            <p className="text-xs text-zinc-400 mt-2">The system will verify if the candidate is older than this age using the provided ZK proof.</p>
                          </div>
                        )}
                        ...
                        {selectedTemplate === 'rank' && (
                          <div>
                            <label className="text-xs font-bold text-indigo-300 uppercase mb-2 block">Maximum University Rank</label>
                            <input type="number" value={maxRank} onChange={(e) => setMaxRank(e.target.value)} className="w-full bg-black/40 border border-indigo-500/30 rounded-lg p-3 text-white focus:outline-none focus:border-indigo-500 transition-all" />
                            <p className="text-xs text-zinc-400 mt-2">Verify if the candidate's university is ranked within this top N list.</p>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-white/5">
                        ...
                        <div className="flex gap-2">
                          <button onClick={handleVerifyZKP} disabled={isVerifying || !zkpInput} className="px-6 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                            {isVerifying ? "Verifying..." : "Verify Proof"}
                          </button>
                          <button onClick={handleVerifyAllCredentials} className="px-6 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-lg transition-all">Verify All Credentials</button>
                        </div>
                      </div>
                      ...
                      {verificationDetails && (
                        <div className="mt-4 bg-black/30 border border-white/5 rounded-lg p-4">
                          <h4 className="text-sm font-bold mb-2">Verification Response</h4>
                          <div className="text-xs font-mono text-zinc-300 space-y-2">
                            {/* If backend returns tokenId at top-level */}
                            {verificationDetails.tokenId && (
                              <div className="flex items-center justify-between bg-white/2 p-2 rounded">
                                <span className="text-zinc-400">Token ID</span>
                                <span className="truncate">{verificationDetails.tokenId}</span>
                              </div>
                            )}

                            {/* Render `verification` object if present (sample response has this) */}
                            {verificationDetails.verification && (
                              <div className="grid grid-cols-2 gap-2">
                                {Object.entries(verificationDetails.verification).map(([k, v]) => (
                                  <div key={k} className="flex items-center justify-between p-2 bg-white/2 rounded">
                                    <span className="capitalize text-zinc-400 text-xs">{k.replace(/([A-Z])/g, ' $1')}</span>
                                    <span className={`font-mono text-xs ${v ? 'text-green-300' : 'text-red-400'}`}>{String(v)}</span>
                                  </div>
                                ))}
                              </div>
                            )}

                            {/* If backend wrapped data under data or returned isValid directly */}
                            {verificationDetails.data && (
                              <div className="mt-2 text-xs">
                                <div className="text-zinc-400 mb-1">Raw data</div>
                                <pre className="bg-black/40 p-2 rounded text-xs overflow-auto max-h-40">{JSON.stringify(verificationDetails.data, null, 2)}</pre>
                              </div>
                            )}
                          </div>
                        </div>
