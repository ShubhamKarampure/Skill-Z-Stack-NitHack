...
export default function IssuePage() {
  ...
  const handleMint = () => {
    ...
    setTimeout(() => {
      setIsMinting(false);
      setSelectedStudents([]);
      alert(`Minting Sequence Initiated for ${selectedStudents.length} students.`);
    }, 2500);
  };
  ...
  const filteredStudents = students.filter((s) => {
    ...
    const emailMatch = s.studentId?.email?.toLowerCase().includes(query) || false;
            ...
            {isLoading ? (
              ...
              filteredStudents.map((student) => {
                ...
                const walletDisplay = student.studentWalletAddress
                  ? `${student.studentWalletAddress.slice(0, 6)}...${student.studentWalletAddress.slice(-4)}`
                  : "No Wallet";
